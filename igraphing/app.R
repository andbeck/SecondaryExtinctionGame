...
# simple random example ---- g <- make_ring(10) %>% set_vertex_attr("name",
#value = LETTERS[1:10]) g <- graph_from_adjacency_matrix(PredationMatrix(TL84))

# n <- 6 g <- erdos.renyi.game(n, p.or.m = 0.4, directed = TRUE) %>%
# set_vertex_attr("name", value = LETTERS[1:n]) plot(g) plot(delete_vertices(g,
# 6))
...
t_links <- as_data_frame(g, what = "edges")
colnames(t_links) <- c("consumer", "resource")

# build community
gg <- Community(properties = prop,
                nodes = nn,
                trophic.links = t_links)

## view of initial community by trophic level - test plot ----

# PlotWebByLevel(gg, show.nodes.as = "labels", colour.by = 'resolved.to',
# node.labels = NPS(gg)$node)

# plot(igraph::graph_from_adjacency_matrix(PredationMatrix(gg)))

# example removal
# gg2 <- RemoveNodes(gg, remove = 6, method = "cascade")
# labs2 <- gg2$nodes$node plot(gg2, show.nodes.as = "labels", node.labels = labs2)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("vertexToDelete", "Species to delete",
                  choices = NPS(gg)$node),
      actionButton("goButtonDelete", "Evaluate"),
      shinyjs::useShinyjs(),
      shinyjs::extendShinyjs(text = "shinyjs.refresh_page = function() { location.reload(); }",
